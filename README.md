...
This creates `release/` with a double-clickable `Flying Solo Contract Review.app`, a companion stop app, and a short setup guide. The package includes the local Node runtime and installed dependencies, so Anna does not need Node.js or Terminal. Her working data is written to `~/Library/Application Support/Flying Solo Contract Review/`, outside the app bundle, so it survives an app update.
